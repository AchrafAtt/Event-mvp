...
const packMeta: Record<string, { description: string; capacity: string; popular?: boolean }> = {
    Base: {
        description: "Organisation & decoration essentielle pour votre evenement.",
        capacity: "Jusqu'a 10 personnes",
    },
    Standard: {
        description: 'Organisation complete avec decoration personnalisee et mise en place.',
        capacity: "Jusqu'a 20 personnes",
        popular: true,
    },
    Premium: {
        description: 'Experience luxe - decoration florale premium, traiteur, photographe.',
...
export default function ReservationOfferPrice({ offerDefaults }: Props) {
    const [selectedOffer, setSelectedOffer] = useState<'Base' | 'Standard' | 'Premium'>(() => {
        ...
        if (stored === 'Base' || stored === 'Standard' || stored === 'Premium') {
            return stored;
        }
    ...
    const recap = useMemo(() => {
        ...
        return {
            ...
            adresse: sessionStorage.getItem('ghozel_adresse_event') ?? sessionStorage.getItem('ghozel_adresse') ?? '—',
            ...
            accessoires: parseArray(sessionStorage.getItem('ghozel_accessoires')),
            extra: parseNumber(sessionStorage.getItem('ghozel_personnes_supplementaires')),
        };
    }, []);
                ...
                <main className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-8 px-4 pt-10 pb-16 md:grid-cols-[1fr_400px] md:px-8">
                    <section>
                        ...
                        <h1 className="font-['Cormorant_Garamond'] text-4xl leading-tight font-light md:text-6xl">
                            Choisissez votre <em className="text-primary font-normal italic">offre</em>
                        </h1>
                        <p className="text-text-secondary mt-3 mb-8 max-w-3xl text-sm leading-7 md:text-base">
                            Selectionnez le pack qui correspond a votre evenement. Le prix total est calcule automatiquement.
                        </p>

                        <p className="text-text-secondary mb-4 text-[11px] tracking-[0.14em] uppercase">Nos formules</p>
                        <div className="space-y-3">
                            {(['Base', 'Standard', 'Premium'] as const).map((offer) => {
                                const selected = selectedOffer === offer;
                                const meta = packMeta[offer];
                                const price = offerDefaults.pack_prices[offer] ?? 0;
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-4">
                                            <div>
                                                ...
                                                <p className="text-text-secondary mt-2 text-sm leading-6">{meta.description}</p>
                                                <p className="text-text-secondary mt-2 text-[11px] tracking-[0.08em] uppercase">
                                                    {meta.capacity}
                                                </p>
                                            </div>
                        ...
                        {recap.extra > 0 && (
                            <div className="border-accent/60 bg-accent/20 mt-6 flex items-center justify-between rounded-xl border px-5 py-4">
                                <p className="text-sm">
                                    Personnes supplementaires : <span className="font-semibold">{recap.extra}</span> x 250 DH
                                </p>
                    ...
                    <aside className="md:sticky md:top-24">
                        <div className="border-border overflow-hidden rounded-2xl border bg-white shadow-sm">
                            <div className="bg-text-primary p-5 text-white">
                                <p className="font-['Cormorant_Garamond'] text-2xl">Recapitulatif de votre reservation</p>
                                <p className="text-white/70 text-xs">Verifiez chaque detail avant de confirmer</p>
                            </div>
                            ...
                            <div className="space-y-0 px-5 py-4">
                                {[
                                    ...
                                    <div key={key} className="border-border/70 flex items-start justify-between gap-3 border-b py-2.5 last:border-b-0">
                                        <span className="text-text-secondary text-[11px] tracking-[0.1em] uppercase">{key}</span>
                                        <span className="text-right text-sm font-semibold">{value}</span>
                                    </div>
                                ))}

                                <div className="border-border/70 flex items-start justify-between gap-3 border-b py-2.5">
                                    <span className="text-text-secondary text-[11px] tracking-[0.1em] uppercase">Couleurs</span>
                                ...
                                <div className="border-border/70 flex items-start justify-between gap-3 py-2.5">
                                    <span className="text-text-secondary text-[11px] tracking-[0.1em] uppercase">Accessoires</span>
                                    <div className="flex max-w-[150px] flex-wrap justify-end gap-1">
                                        {recap.accessoires.length > 0 ? (
                                            recap.accessoires.map((item) => (
                                                <span key={item} className="bg-bg-card text-primary rounded-full px-2 py-0.5 text-[10px]">
                                                    {item}
                                                </span>
                                            ))
                                        ) : (
                            ...
                            <div className="border-border bg-bg-global border-t px-5 py-4">
                                <div className="mb-1 flex items-center justify-between text-sm">
                                    <span className="text-text-secondary">Pack {selectedOffer}</span>
                                    <span className="font-semibold">{packBase.toLocaleString('fr-FR')} DH</span>
                                </div>
                                {extrasAmount > 0 && (
                                    <div className="mb-1 flex items-center justify-between text-sm">
                                        <span className="text-text-secondary">Personnes supp. ({recap.extra} x 250)</span>
                                        <span className="font-semibold">{extrasAmount.toLocaleString('fr-FR')} DH</span>
                                    </div>
                                )}
                                <div className="border-border mt-2 flex items-center justify-between border-t pt-2">
                                    <span className="text-sm font-semibold uppercase">Prix total</span>
                                    <span className="font-['Cormorant_Garamond'] text-primary text-4xl font-semibold">
                                        {total.toLocaleString('fr-FR')} DH
                                    </span>
                                </div>
                                <div className="bg-bg-card mt-2 flex items-center justify-between rounded-lg px-3 py-2">
                                    <span className="text-primary text-xs font-semibold">Avance a payer (obligatoire)</span>
                                    <span className="text-primary font-semibold">{advance.toLocaleString('fr-FR')} DH</span>
                                </div>
                                <div className="mt-2 flex items-center justify-between text-sm">
                                    <span className="text-text-secondary">Reste a payer</span>
                                    <span className="font-semibold">{remaining.toLocaleString('fr-FR')} DH</span>
                                </div>
                            </div>
                        </div>
                    </aside>
