...
export default function ReservationEventDetails({
    ...
    const [form, setForm] = useState<EventDetailsForm>(() => {
        ...
        return {
            ...
            horaire:
                sessionStorage.getItem('ghozel_horaire') ??
                eventDetailsDefaults.horaire,
            adresse_event:
                sessionStorage.getItem('ghozel_adresse_event') ??
                sessionStorage.getItem('ghozel_adresse') ??
                eventDetailsDefaults.adresse_event,
            ville:
                sessionStorage.getItem('ghozel_ville') ??
                eventDetailsDefaults.ville,
            nombre_personnes:
                Number.parseInt(
                    sessionStorage.getItem('ghozel_nombre_personnes') ?? '20',
                    10,
                ) || eventDetailsDefaults.nombre_personnes,
            texte_personnalise:
                sessionStorage.getItem('ghozel_texte_personnalise') ?? '',
    ...
    const validate = (): Partial<Record<FormField, string>> => {
        ...
        if (
            !Number.isInteger(form.nombre_personnes) ||
            form.nombre_personnes < 5
        ) {
    ...
    const updateGuests = (delta: number): void => {
        setForm((current) => ({
            ...current,
            nombre_personnes: Math.min(
                500,
                Math.max(5, current.nombre_personnes + delta),
            ),
        }));
    };
                ...
                <main className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-8 px-4 pt-10 pb-16 md:grid-cols-[1fr_360px] md:px-8">
                    <section>
                        ...
                        <p className="mt-3 mb-10 max-w-3xl text-sm leading-7 text-text-secondary md:text-base">
                            Precisez les informations pratiques de votre
                            evenement.
                        </p>
                        ...
                        >
                            ...
                            <div className="md:col-span-2">
                                ...
                                <input
                                    ...
                                    onChange={(event) =>
                                        setForm((current) => ({
                                            ...current,
                                            adresse_event: event.target.value,
                                        }))
                                    }
                                    className={`w-full rounded-xl border bg-white px-4 py-3 transition-all outline-none ${
                                        errors.adresse_event
                                            ? 'border-error'
                                            : 'border-border focus:border-primary'
                                    }`}
                            ...
                            <div className="md:col-span-2">
                                <label className="mb-2 block text-[11px] tracking-[0.12em] text-text-secondary uppercase">
                                    Texte personnalise{' '}
                                    <span className="text-text-secondary/80 normal-case">
                                        (optionnel)
                                    </span>
                                </label>
                                <textarea
                                    ...
                                    className="min-h-24 w-full rounded-xl border border-border bg-white px-4 py-3 outline-none focus:border-primary"
                    ...
                    <aside className="md:sticky md:top-24">
                        <div className="rounded-2xl border border-border bg-white p-6 shadow-sm">
                            ...
                            <div className="flex items-center justify-between border-b border-border/70 py-3">
                                ...
                                <span
                                    className={`text-sm ${form.date ? 'font-semibold' : 'text-text-secondary italic'}`}
                                >
                                    {formattedDate}
                                </span>
                            </div>
                            <div className="flex items-center justify-between border-b border-border/70 py-3">
                                ...
                                <span
                                    className={`text-sm ${form.horaire ? 'font-semibold' : 'text-text-secondary italic'}`}
                                >
                                    {form.horaire || '—'}
                                </span>
                            </div>
