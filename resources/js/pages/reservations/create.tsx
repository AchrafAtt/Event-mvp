...
const eventVisuals: Record<string, EventVisual> = {
    ...
    'Acte de mariage': {
        icon: '🤝',
        description:
            "La ceremonie civile merite elle aussi d'etre magnifiee. Creez un moment solennel et beau des la signature.",
        price: '1 600 DH',
    ...
    'Fete Henna': {
        icon: '🌿',
        description:
            'La tradition marocaine revisitee avec gout — couleurs chaudes, decoration orientale et ambiance festive.',
        price: '2 500 DH',
    ...
    Anniversaire: {
        icon: '🎂',
        description:
            'Chaque anniversaire est unique. Nous creons une celebration sur mesure, coloree ou elegante.',
        price: '2 500 DH',
...
export default function ReservationCreate({
        ...
        () =>
            eventTypes.map((eventType) => ({
                ...
                visual: eventVisuals[eventType.value] ?? {
                    icon: '🎉',
                    description:
                        'Choisissez ce type evenement pour continuer la reservation.',
                    price: 'Sur devis',
                    iconClassName: 'bg-bg-card',
                },
            })),
    ...
    const selectedCard =
        cards.find((card) => card.value === selectedType) ?? null;
            ...
            <div className="min-h-screen bg-bg-global text-text-primary">
                ...
                <main className="mx-auto w-full max-w-6xl px-4 pt-10 pb-28 md:px-8">
                    ...
                    <p className="mt-3 mb-10 max-w-3xl text-sm leading-7 text-text-secondary md:text-base">
                        Selectionnez l&apos;occasion que vous souhaitez
                        celebrer. Chaque evenement a ses propres formules
                        adaptees.
                    </p>
                                ...
                                <button
                                    ...
                                    className={`relative flex min-h-[320px] flex-col rounded-3xl border-2 bg-white p-7 text-left transition-all ${
                                        isSelected
                                            ...
                                            : 'border-border hover:-translate-y-1 hover:border-primary-soft'
                                    }`}
                                >
                                    <div
                                    ...
                                    <div
                                        className={`${card.visual.iconClassName} mb-5 flex h-[72px] w-[72px] items-center justify-center rounded-2xl text-4xl`}
                                    >
                                        {card.visual.icon}
                                    </div>
                                    <h2 className="font-['Cormorant_Garamond'] text-4xl leading-none font-medium">
                                        {card.label}
                                    </h2>
                ...
                <div
                    className={`fixed right-0 bottom-0 left-0 border-t border-border/80 bg-white/95 px-4 py-4 backdrop-blur-md transition-transform duration-300 md:px-8 ${
                        selectedCard ? 'translate-y-0' : 'translate-y-full'
                    }`}
                >
                    <div className="mx-auto flex w-full max-w-6xl flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-bg-card text-xl">
                                {selectedCard?.visual.icon ?? '🎉'}
                            </div>
                            <div>
                                <p className="text-[11px] tracking-[0.1em] text-text-secondary uppercase">
                        ...
                        <button
                            ...
                            className="rounded-full bg-primary px-8 py-3 text-sm font-semibold tracking-[0.1em] text-white uppercase transition-all hover:-translate-y-0.5 hover:bg-primary hover:brightness-90 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                            Continuer
                        </button>
                    </div>
